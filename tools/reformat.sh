...
if git grep -q '^[[:space:]]*//go:embed ' -- . ':!pkg/frontend'; then
  bazel build $(git grep '^[[:space:]]*//go:embed ' -- . ':!pkg/frontend' | sed -e 's|\(.*\)/.*//go:embed |//\1:|; s|"||g; s| .*||' | sort -u)
  git grep '^[[:space:]]*//go:embed ' -- . ':!pkg/frontend' | sed -e 's|\(.*\)/.*//go:embed |\1/|' | while read o; do
