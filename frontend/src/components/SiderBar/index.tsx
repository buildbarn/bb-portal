...
const SiderBar: React.FC<Props> = ({
  ...
  const [siderWidth, setSiderWidth] = useState<number>(() => {
    const cachedExpandedState = window.localStorage.getItem(
      SIDER_EXPANDED_WIDTH_LOCAL_STORAGE_KEY,
    );
      ...
      <Menu
        ...
        onMouseMove={() => {
          const updatedSiderWidth =
            expandedWidth && expandedWidth > DEFAULT_SIDER_WIDTH
              ? expandedWidth
              : DEFAULT_SIDER_WIDTH;
          setSiderWidth(updatedSiderWidth);
          localStorage.setItem(
            SIDER_EXPANDED_WIDTH_LOCAL_STORAGE_KEY,
            updatedSiderWidth.toString(),
          );
        }}
        onMouseLeave={() => {
