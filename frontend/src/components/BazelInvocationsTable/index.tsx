...
const BazelInvocationsTable: React.FC<Props> = ({ height }) => {
  const [variables, setVariables] =
    useState<FindBazelInvocationsQueryVariables>({
      first: PAGE_SIZE,
    });

  const { loading, data, previousData, error } = useQuery(
    FIND_BAZEL_INVOCATIONS_QUERY,
    {
      variables,
      pollInterval: 120000,
    }
  ...
  const onChange: TableProps<BazelInvocationNodeFragment>["onChange"] =
    useCallback(
      (
        ...
        if (filters["build"]?.length) {
          wheres.push({
            hasBuildWith: [{ buildUUID: filters["build"][0].toString() }],
          });
        }
  ...
  if (error) {
    ...
    const bazelInvocationNodes =
      activeData?.findBazelInvocations.edges?.flatMap((edge) => edge?.node) ??
      [];
    const bazelInvocationNodeFragments = bazelInvocationNodes.map((node) =>
      getFragmentData(BAZEL_INVOCATION_NODE_FRAGMENT, node)
    );
    dataSource = bazelInvocationNodeFragments.filter(
      (nodeFragment): nodeFragment is BazelInvocationNodeFragment =>
        !!nodeFragment
    );
  }
