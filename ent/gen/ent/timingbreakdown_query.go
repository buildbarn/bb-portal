...
func (tbq *TimingBreakdownQuery) loadChild(ctx context.Context, query *TimingChildQuery, nodes []*TimingBreakdown, init func(*TimingBreakdown), assign func(*TimingBreakdown, *TimingChild)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(timingchild.FieldTimingBreakdownID)
	}
	...
	for _, n := range neighbors {
		fk := n.TimingBreakdownID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "timing_breakdown_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tbq *TimingBreakdownQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tbq.ctx.Fields; len(fields) > 0 {
		...
		if tbq.withExecutionInfo != nil {
			_spec.Node.AddColumnOnce(timingbreakdown.FieldExecutionInfoID)
		}
	}
